 ...
 * @file: 
 * @Brief: 
 * @FilePath: \D3project\D3_zu\src\utilities\crc32\utilities_crc32.c
 * @PROJECT: D3
 * @COMPILER: GCC
 * @HW PLATFORM: ARM
 * @Author: Wang ShiLong
 ...
 * @LastEditTime: 2023-01-25 11:26:24
 */

#include "utilities_crc32.h"
...
uint32_t UTILITY_CRC32_Calculate(uint32_t init_value, const uint8_t data[], uint32_t data_length, bool is_init_enable, bool result_xor_enable)
{
    ...
    {
        result = (uint32_t)(crc32_table[(result & 0x000000FF) ^ data[i]] ^ (result >> 8));
    }

    crc32_val = result;
    if (result_xor_enable)
    {
        crc32_val = ~crc32_val;
    }
    return (uint32_t)(crc32_val);
}
